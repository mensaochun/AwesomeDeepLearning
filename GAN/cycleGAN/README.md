...
1. 在GAN的模型上增加了信息论扩展，可以以一种完全无监督的方式来学习分离的表示，这个任务中会最大化潜变量和样本观察值（observation）之间的的互信息。
2. 推导处理互信息的下界，因此可以很好地进行优化。

我们希望输入的高斯噪声z能够控制输出的结果。常规的GAN是没办法做这个事情的，特定的维度很难说能控制什么。但是INFOGAN就可以每个维度都代表一定的意义。

![1540871631628](pics/1540871631628.png)
...
这个auto-encoder并不是常规的auto-encoder。而是返回来的autoencoder。

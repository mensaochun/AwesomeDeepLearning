...
FCN的upconvolution层+shortcut connections产生的分割图比较粗糙，因此SegNet增加了更多的shortcut connections。不过，SegNet并不是直接将encoder的特征进行直接复制，而是复制maxpooling中的indices进行复制，这使得SegNet的效率更高。
...
2. 提出上下文木块（context module），使用空洞卷积来进行多尺度信息的的整合。
...
- 将VGG网络的最后两个pooling层给拿掉了，之后的卷积层被dilated 卷积取代。并且在pool3和pool4之间的空洞卷积的rate=2，pool4之后的空洞卷积的rate=4。作者将这种架构称为前端（front-end）。

...
​	语义分割是在像素级别上的分类，属于同一类的像素都要被归为一类，因此语义分割是从像素级别来理解图像的。比如说如下的照片，属于人的像素都要分成一类，属于摩托车的像素也要分成一类，除此之外还有背景像素也被分为一类。注意语义分割不同于实例分割，举例来说，如果一张照片中有多个人，对于语义分割来说，只要将所由人的像素都归为一类，但是实例分割还要将不同人的像素归为不同的类。也就是说示例分割比语义分割更进一步。

![motor](/home/pi/stone/Notes/DeepLearning/segmentaion/pics/motor.jpg)

图1(a). 原图

![21_class](/home/pi/stone/Notes/DeepLearning/segmentaion/pics/21_class.png)
...
最初的深度学习方法应用与图像分割就是Patch classification。Patch classification方法，顾名思义，图像是切成块喂给深度模型的，然后对像素进行分类。使用图像块的主要原因是因为全连接层需要固定大小的图像。
...
第一中架构就是encoder-decoder架构。encoder由于pooling逐渐减少了空间维度，而decoder逐渐恢复空间维度和细节信息。通常从encoder到decoder还有shortcut connetction（捷径连接，也就是跨层连接）。其中U-net就是这种架构很流行的一种，如下图：

![unet](/home/pi/stone/Notes/DeepLearning/segmentaion/pics/unet.png)

**空洞卷积**

第二种架构叫做dilated/atrous （空洞卷积），这种结构代替了pooling，一方面它可以保持空间分辨率，另外一方面它由于可以扩大感受野因而可以很好地整合上下文信息。如下图：

![dilated_conv](/home/pi/stone/Notes/DeepLearning/segmentaion/pics/dilated_conv.png)

**条件随机场**

除了以上思路，还有一种对分割结果进行后处理的方法，那就是条件随机场(Conditional Random Fields (CRFs))后处理用来改善分割效果。DeepLab系列文章基本都采用这种后处理方法，可以较好地改善分割结果，如下图：

![CRF](/home/pi/stone/Notes/DeepLearning/segmentaion/pics/CRF.png)
...
![summary](/home/pi/stone/Notes/DeepLearning/segmentaion/pics/summary.png)

**各方法的详细信息**

![table-method](/home/pi/stone/Notes/DeepLearning/segmentaion/pics/table-method.png)
...
通过两种方式来进行多尺度的处理：1. 将原始图像的多种尺度喂给网络进行训练。2.通过平行的不同空洞率的空洞卷积层来获得。

通过全连接条件随机场来进行后处理，以改善分割结果。

![Selection_002](/home/pi/stone/Notes/DeepLearning/segmentaion/pics/dilated_conv.png)
...
![refinenet](/home/pi/stone/Notes/DeepLearning/segmentaion/pics/refinenet.png)

![refineblock](/home/pi/stone/Notes/DeepLearning/segmentaion/pics/refineblock.png)
